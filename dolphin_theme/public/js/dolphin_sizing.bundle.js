  ...
  function blockTable(d, edit) {
    ...
    if (edit) {
      ...
      h.push('<div class="bar" style="flex-direction:column;align-items:stretch;gap:6px">' +
             ...
             /* 5 Sep 2026: "sizes have to be all a,b,c that is the whole idea to
                overrule last minute and write back". This list was built from
                d.bands - the SORTING thresholds - so with B switched off as a
                threshold the screen offered A and C only and a B could never be
                agreed. The sort still follows the bands; the person choosing by
                hand gets every size. d.size_options comes from sizing.panel. */
             '<select data-dsz="szval" disabled>' +
             (d.size_options && d.size_options.length
                ? d.size_options
                : (d.bands || []).map(function (x) { return x.size; })
             ).map(function (s) {
    ...
    $sec.find('[data-dsz="range"]').on('click', function () {
      rangeDialog({
        ...
        apply: function (groups) {
          ...
          /* the same full list the Size dropdown offers, so a range line
             reading "B" is understood as the size B and not as a grade. */
          var sizes = (d.size_options && d.size_options.length
                        ? d.size_options
                        : (d.bands || []).map(function (x) { return x.size; })
