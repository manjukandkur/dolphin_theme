...
(function(){
  ...
  function dolphinFind(q,cb){q=(q||'').trim();if(!q){cb([]);return;}var out=[],p=0;function d(){p--;if(p<=0)cb(out);}
    ...
    function child(cdt,kf,val,pdt,grp,mk){p++;fetch('/api/method/frappe.client.get_list?doctype='+encodeURIComponent(cdt)+'&filters='+encodeURIComponent(JSON.stringify([[kf,'=',val],['parenttype','=',pdt]]))+'&fields='+encodeURIComponent(JSON.stringify(['parent']))+'&limit_page_length=8',{credentials:'include'}).then(function(r){return r.json();}).then(function(j){var seen={};(j.message||[]).forEach(function(x){if(!x.parent||seen[x.parent])return;seen[x.parent]=1;out.push({t:grp,l:pdt+' · '+x.parent,s:'',actions:mk(x.parent)});});d();}).catch(d);}
