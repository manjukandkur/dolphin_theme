...
@frappe.whitelist()
def size_options(doc=None):
    """EVERY size a person may put on a block by hand - not only the ones the
    thresholds sort into.

    5 Sep 2026, his words: "sizes have to be all a,b,c that is the whole idea to
    overrule last minute and write back... we assume that a block is c size based
    on a thumb rule like 300*70*200 but due to its other dimension being
    favourable buyer will buy it under A... in that case we might negotiate for a
    B size so we need this flexibility to override".

    The dropdown used to be built from `bands` - the sorting thresholds - and B
    is switched off as a THRESHOLD (its minimums are 0x0x0, so an active B would
    swallow every block the A band rejected). Switched off as a threshold is not
    the same as unavailable to a person, and conflating the two is what left the
    screen offering A and C only, with no way to agree a B.

    So: the sort still follows the active bands and nothing about it changes.
    This is the CHOICE list, and it holds every house size category in its own
    order, whether or not it is a sorting band, plus anything already sitting on
    a block so no existing value is unselectable."""
    seen, out = set(), []

    def add(v):
        v = _s(v)
        if v and v.lower() != "custom" and v not in seen:
            seen.add(v)
            out.append(v)
    ...
    try:
        rows = frappe.get_all(
            "Granite Size Category",
            fields=["name", "size_category_name", "sort_order", "buyer",
                    "export_consignee", "variation_label"],
            limit_page_length=0)
        rows = [r for r in rows
                if not _s(r.get("buyer")) and not _s(r.get("export_consignee"))
                and not _s(r.get("variation_label"))]
        rows.sort(key=lambda r: (cint(r.get("sort_order")) or 99))
        for r in rows:
            add(_s(r.get("size_category_name")) or _s(r.get("name")))
    except Exception:
        pass

    for letter in ("A", "B", "C"):      # the three he named, always offered
        add(letter)

    if doc is not None:                 # never orphan a value already written
        try:
            for b in _blocks_of(doc):
                add(b.get(SIZE_FIELD))
        except Exception:
            pass
    return out
...
def _size_rank(doc):
    """How the sizes stack up, for deciding whether a change is a PROMOTION.

    The order comes from the choice list, so a block moving C -> B counts as
    moving up and asks who agreed it - which it would not if the ranking only
    knew the two sorting bands."""
    order = {}
    for i, s in enumerate(size_options(doc)):
        order[s] = i
    # Anything the thresholds know but the master does not still needs a place,
    # and it goes at the end rather than colliding with a rank already taken.
    for c in bands_for(doc):
        key = _s(c.get("size_category_name")) or _s(c.get("name"))
...
def panel(doctype=None, name=None):
    ...
    return {
        ...
        # 6 Sep 2026, his question: "undo reset didnt work?". It did not, and
        # this line is why. Every size change made on a Shipping Document is
        # written on its LOT (see _write_target), so the undo snapshot is kept
        # on the lot - but this asked the SHIPPING DOCUMENT whether there was
        # anything to undo. There never was, so the Undo button never appeared.
        # The undo belongs where the writing landed: with the owner.
        "undo": undo_available(tgt.doctype, tgt.name),
        ...
        "bands": [{"size": _s(b.get("size_category_name")) or _s(b.get("name")),
                  ...
                  for b in bands],
        # The sorting thresholds are `bands`. THIS is what a person may choose
        # from by hand - every size, so a block can be agreed up to B or A
        # whatever the thumb rule sorted it as. See size_options.
        "size_options": size_options(doc),
        "standard": std,
        "tolerance_cm": tol,
        "blocks": blocks,
...
def undo_last(doctype=None, name=None, person=None):
    ...
    6 Sep 2026: it follows the same redirect the writes do, so pressing Undo on
    a Shipping Document undoes the change on the lot where it was actually
    written. Without this it looked at the document in front of him, found no
    snapshot, and said there was nothing to undo."""
    _r = _redirect(doctype, name, dict(person=person), undo_last)
    if _r is not None:
        return _r
    doc = _doc_for_panel(doctype, name)
    raw = _s(doc.get("size_grade_undo"))
...
def set_sizes(doctype=None, name=None, rows=None, to_size=None, agreed_by=None,
    ...
    # 5 Sep 2026: ranked by the full size list, not only by the sorting bands,
    # so agreeing a block up from C to B is recognised as a promotion and asks
    # who agreed it. See _size_rank.
    order = _size_rank(doc)
